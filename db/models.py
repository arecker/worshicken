...
class Break(models.Model):
    id = models.UUIDField(auto_created=True, primary_key=True, verbose_name='ID', default=uuid.uuid4)
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='breaks')
    start = models.DateField()
    end = models.DateField()
    reason = models.CharField(max_length=255, blank=True, help_text='Optional: e.g., Vacation')

    def __str__(self):
        msg = f'{self.start} - {self.end}'
...
class SongChart(models.Model):
    ...
    def __str__(self):
        return f'{self.get_key_display()}{self.get_pitch_display()}'
...
class SetTemplate(models.Model):
    ...
    instruments = models.ManyToManyField('Instrument', related_name='templates')
